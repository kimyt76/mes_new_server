...
public class PurchaseOrderController {
    ...
    public ResponseEntity<Resource>  printOut(@RequestBody Map<String, Object> map) throws  Exception {
        ...
        for(int i = 0; i < purOrderIds.size(); i++) {
            ...
            poSheet.setOrderDate(poVo.getPurOrderDate().toString());
            poSheet.setDeliveryDate(poVo.getDeliveryDate().toString());
    ...
    public RestResponse<T> orderMail(@RequestBody MailVo vo) throws Exception {
        ...
        List<PurchaseOrderSheet> poSheetList = new ArrayList<>();
        PurchaseOrderRequestVo reqVo = purchaseOrderService.getPurchaseOrderInfoMail(map);
        List<CommonVo> commonList = commonService.getCodeList("appr_line");
        String line1 = commonList.get(0).getCodeNm();
        String line2 = commonList.get(1).getCodeNm();
        String line3 = commonList.get(2).getCodeNm();
        ...
        try {
            ...
            files.put("발주서("+ poVo.getPurOrderDate().toString()+"-"+poVo.getSeq()+ ").pdf", bds);

            PoSheetMailVo psVo = new PoSheetMailVo();
            psVo.setCustomerName(customer.getCustomerName());
            psVo.setOrderNo(poVo.getPurOrderDate().toString()+"-"+poVo.getSeq());
            psVo.setOrderDate(poVo.getPurOrderDate().toString());
            psVo.setSenderEmail(vo.getFrom());
            psVo.setMemo(vo.getMemo());

            vo.setMailId("1");
            vo.setSubject("(주)진코스텍으로부터 발주서(" + poVo.getPurOrderDate().toString()+"-"+poVo.getSeq() + ")가 도착했습니다.");
            ...
            throw new BusinessException(ErrorCode.FAIL_MAIL);
        }
    }
